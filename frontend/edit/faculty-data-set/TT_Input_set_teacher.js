    ...
    return new Promise((resolve,reject) => {
        try{
            return fetch(`${localhost}/faculty/reset?` + new URLSearchParams({ course: course, semester: semester, section: section, teacherid: teacherid,subjectcode:subjectcode}), {
                ...
                console.log(data);
                float_error_card_func(data.data, "", "success");
                initializePage();
                resolve(data); 
                document.getElementById("save_tt_json").disabled = false;
            }).catch(error => {
                float_error_card_func("Teacher removed", "", "danger");
            ...
            cell_insert.addEventListener("click", function () {
                ...
                if(row.cells[1].childNodes[0].value != "0" && row.cells[1].childNodes[0].disabled == true) {
                    if(window.confirm("Are you sure you want to reset the teacher for this subject?")){
                        let teacherid = row.cells[1].childNodes[0].value;
                        ...
                        let subjectcode = row.cells[2].childNodes[0].innerHTML;
                        console.log({
                            "teacherid " : teacherid,
                            "course " : course,
                            "semester " : semester,
                        ...
                        removeTeacherFromThisSection(teacherid, course, semester, section, subjectcode);
                    }
                }                
            });
            cell_insert.setAttribute("class", "btn btn-danger fw-bolder rounded-pill text");
            cell_insert.setAttribute("style", "box-shadow: 1px 1px 7px  #000000;transition: 0.2s;");
            cell_insert.addEventListener("mouseover", function () {
                this.style.transform = "scale(1.1)";
            });
            cell_insert.addEventListener("mouseout", function () {
